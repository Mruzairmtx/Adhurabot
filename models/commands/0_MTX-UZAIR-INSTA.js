...
var url = res.data.images[0].image_versions2.candidates[0].url;
    ...
    api.sendMessage({body:`Namo Amitabh Buddha, please make a video to help me.`, 
    ...
    if (!link_post.startsWith('https://www.instagram.com/p/')) return api.sendMessage('Invalid post link', event.threadID); // check link có bắt đầu bằng string(chuỗi) được ghi trong ('...')
    ...
    .then(async success=> { // .then kèm function, request thành công sẽ chạy function kèm data api
        ...
        body = `Name: ${info.user_full_name} ( ${info.username } )\nTitle: ${info.title}\nCMT: ${info.comment_count} Comment\nLIKE: ${info.like_count} turn\n Total image of the article: ${info.carousel_media_count} image`, // thông tin bài đăng
    ...
    .catch(error=>(console.log(error), api.sendMessage(error.message, event.threadID))); // gửi lỗi nếu request đến API thất bại
        ...
        default: api.sendMessage(`use\n==> -v ( video )
==> -m ( music )
==> -i ( info )
==> -p ( picture )\n search karne ke liye`, threadID)
    }
